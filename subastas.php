...
require'database.php';
	...
	foreach($sql2 as $row){
		...
		$fecha_comparar = $interval->format('%R%d días %H:%I:%s');
				...
				echo"Precio inicial:$".number_format($f12)." <br/>";
				echo"Mayor Subasta Hasta el momento:$".number_format($f12);
				echo"<table border=0 cellspacing=0 cellpadding=0>
					<tr>
					   <td><div id='cuenta'> <h1 style='color:red' > ".$fecha_comparar." </h1></div></td>
					</tr>
					</table>";
				if(isset($_SESSION['id'])){
					if($_SESSION['id'] == $f20){
						...
						echo'<button type="submit" class=" btn btn-success" name="botonn" id="botonn">Ofertar ahora</button>';
						echo'</form>';							
						}
					}
				}
...
$(document).ready(function() {
    ...
    $('#PAG').load('subastas.php');//actualizas el div
   }, 1000 );
});
